...
# NOTE: Database checks are now prioritized first to catch critical issues early
...
# 1. Check if Cloud SQL Proxy on backend is running (prerequisite for DB checks)
...
# 2. Check database connectivity from backend (critical check - prioritize first)
...
# 3. Check if database tables exist and are populated (critical check)
...
# 5. Check if application code is properly copied to instances
log "5. Checking if application code is properly copied to instances..."
...
# 6. Check if Node.js dependencies are installed
log "6. Checking if Node.js dependencies are installed..."
...
# 7. Check if backend process is listening on port 3000
log "7. Checking if backend process is listening on port $BACKEND_PORT..."
...
# 8. Check if frontend process is listening on port 3030
log "8. Checking if frontend process is listening on port $FRONTEND_PORT..."
...
# 9. Check PM2 status for backend
log "9. Checking PM2 status for backend..."
...
# 10. Check PM2 status for frontend
log "10. Checking PM2 status for frontend..."
...
# 11. Load-balancer ↔ FE VM Health Check
log "11. Checking Load-balancer ↔ Frontend VM health..."
...
# 12. Test FE → BE connectivity
log "12. Testing Frontend → Backend connectivity..."
...
# 13. Smoke test via Load Balancer
log "13. Testing via Load Balancer..."
...
# 14. Check backend API endpoints
log "14. Testing backend API endpoints..."
