...
# We need to check for "COUNT(*)" on one line and the count value on another line
if (echo "$DB_TEST" | grep -q "COUNT(*)" && echo "$DB_TEST" | grep -q "40") && ! echo "$DB_TEST" | grep -qi "error" && ! echo "$DB_TEST" | grep -q "Access denied" && ! echo "$DB_TEST" | grep -q "Empty set"; then
